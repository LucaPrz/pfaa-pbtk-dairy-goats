...
The dataset comprises measurements of PFAAs in multiple biological matrices. PFAA concentrations were quantified, with limits of quantification (LOQ) of 0.005 µg/L for milk and 0.5 µg/L for plasma, feces, urine, and organ tissues. Throughout, tissue concentrations are reported as mass per kg (µg/kg) for solid matrices and per volume (µg/L) for plasma, milk, urine, and feces, consistent with the body burden formulation (Eq.~\ref{eq:body_burden}).
...
The daily intake of PFAAs for dairy goats is calculated from time-resolved hay concentrations and per-animal feed intake over the in-vivo feeding study period (days 0–140). For each day $t$, animal $a$, compound and isomer, the intake from hay is given by:

\begin{equation}
    I_a(t) = C_{\text{hay}}(t) \cdot M_{\text{hay},a}(t)
\label{eq:intake}
\end{equation}

where $I_a(t)$ is the daily PFAA intake from hay for animal $a$ (µg/day), $C_{\text{hay}}(t)$ is the hay concentration of the respective compound--isomer at day $t$ (µg/kg), and $M_{\text{hay},a}(t)$ is the individual daily hay intake of animal $a$ (kg/day) from the contaminated batch. Hay concentrations are derived from hay samples and applied as a step function over time based on the sampling day, i.e.\ for model day $t$ the last available hay concentration measured at a sampling day $\leq t$ is used. Daily feed intake trajectories are taken from the animal records and restricted to the exposure period (days 1--56), with intake set to zero outside this window. When the recorded intake for the first exposure day was zero but a positive value was available for day 2, the day-2 intake was used for day 1 to avoid an unrealistically intake-free start of exposure.

For the optimisation and prediction runs, these raw daily intake series are smoothed on the model time grid using a locally weighted regression (LOWESS) filter applied separately to each (animal, compound, isomer) intake curve.

\subsubsection{Urine volume calculation}
\label{subsubsec:urine_volume}

Given the creatinine concentration in the urine, it is possible to calculate the urine volume with the following formula:
...
where $V_{\text{urine}}$ is the urine volume (L/day), $BW$ is body weight (kg), $C_{\text{crea}}$ is the creatinine concentration in urine (mmol/L), and $k_{\text{cr}} = 0.424~\text{mmol/kg}^{0.75}$ is the average daily creatinine excretion scaled allometrically with $BW^{0.75}$ \cite{ref1}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{Figures/urine_volume_distribution.png}
    \caption{Distributions of the estimated urine volumes for each animal. High volume outliers cause skewness in the distributions.}
...
Elimination occurs directly from plasma via three routes: (1) urinary excretion with first-order rate constant $k_{\text{urine}}$; (2) milk excretion, modeled as plasma clearance proportional to milk yield and governed by the milk--plasma partition coefficient $P_{\text{milk}}$; and (3) an additional unspecified elimination pathway with rate $k_{\text{elim}}$.


The unspecified plasma elimination rate $k_{\text{elim}}$ represents the aggregate of poorly characterized or experimentally unobserved loss processes (e.g.\ binding and sequestration outside the measured tissue panel or experimental losses), and its inclusion is motivated by the incomplete experimental mass balance described in \ref{subsec:mass_balance}. Rather than inflating the specific, physiologically interpretable excretion routes ($k_{\text{renal}}$, $k_{\text{feces}}$, milk clearance) beyond what is supported by the corresponding urine, feces and milk data, $k_{\text{elim}}$ provides a flexible sink term whose magnitude is constrained by the plasma and tissue time courses. Accordingly, $k_{\text{elim}}$ should be interpreted purely as a lumped effective loss rate (combining true physiological elimination with experimental and structural loss), rather than as a physiologically interpretable clearance. 
...
\begin{equation}
\frac{\mathrm{d}A_{\text{pla}}}{\mathrm{d}t} = \frac{Q_{\text{hepatic}} + Q_{\text{intestine}}}{PC_{\text{liver}} \cdot V_{\text{liver}}} \cdot A_{\text{liv}} + \sum_{\text{org}} \frac{Q_{\text{org}}}{PC_{\text{org}} \cdot V_{\text{org}}} \cdot A_{\text{org}} - \frac{Q_{\text{total}}}{V_{\text{plasma}}} \cdot A_{\text{pla}} - A_{\text{pla}} \cdot k_{\text{elim}} - A_{\text{pla}} \cdot k_{\text{urine}} - A_{\text{pla}} \cdot k_{\text{milk}}(t)
\label{eq:dA_pla}
\end{equation}
...
\begin{equation}
\frac{\mathrm{d}A_{\text{kidney}}}{\mathrm{d}t} = \frac{Q_{\text{kidney}}}{V_{\text{plasma}}} \cdot A_{\text{pla}} - \frac{Q_{\text{kidney}}}{PC_{\text{kidney}} \cdot V_{\text{kidney}}} \cdot A_{\text{kidney}}
\label{eq:dA_kidney}
\end{equation}
...
\begin{align*}
A_{\text{comp}}        &:\ \text{Mass in compartment (µg)} \\
u(t)                   &:\ \text{Input rate into stomach (µg/d)} \\
k_{\text{sto}}         &:\ \text{Stomach emptying rate (1/d)} \\
k_a                    &:\ \text{Intestinal absorption rate (1/d)} \\
k_{\text{feces}}       &:\ \text{Fecal excretion rate (1/d)} \\
k_{\text{ehc}}         &:\ \text{Enterohepatic recirculation rate (1/d)} \\
k_{\text{urine}}       &:\ \text{Urinary excretion rate (1/d)} \\
k_{\text{elim}}        &:\ \text{Plasma elimination rate (1/d)} \\
Q_{\text{org}}         &:\ \text{Blood flow to organ (L/d)} \\
V_{\text{org}}         &:\ \text{Volume of organ (L)} \\
PC_{\text{org}}        &:\ \text{Generic organ partition coefficient (dimensionless)} \\
P_{\text{milk}}        &:\ \text{Milk--plasma partition coefficient (dimensionless)} \\
\text{Lact}(t)         &:\ \text{Milk yield (kg/day) at time $t$} \\
k_{\text{milk}}(t)     &:\ \text{Time-dependent effective milk clearance rate (1/d)}
\end{align*}
...
where $N_i$ is the number of time points contributing to the loss for compartment $i$. Throughout the loss and identifiability sections, $\varepsilon$ without indices denotes a small deterministic numerical constant ($10^{-6}$) used to stabilise logarithms and finite-difference gradients; stochastic measurement noise is denoted separately as $\xi_{ij,t}$ in the variance decomposition.

Following the initial fit, effective error standard deviations ($\sigma_i$) are estimated separately for each compartment $i$ from all residuals of uncensored observations of all compound-isomer pairs:
...
The leading term $-\log(y_{i,j}^{\text{obs}})$ corresponds to the Jacobian of the change of variables from $\log(y)$ back to $y$ under the log-normal error model in \Eqref{eq:likelihood}; it is therefore a standard component of the log-density and not an additional penalty term.
...
where $\bar{\log}_{10}(\hat{\theta}_{j}) = \frac{1}{n} \sum_{i=1}^{n} \log_{10}(\hat{\theta}_{(-i),j})$ is the mean of the jackknife estimates in log-space for parameter $j$, serves as a measure of parameter uncertainty. The prefactor $\frac{n-1}{n}$ is the standard finite-sample correction for jackknife variance and yields an (approximately) unbiased variance estimate under regularity conditions. 
\subsubsection{Monte Carlo sampling}
\label{subsubsec:monte_carlo}

To propagate parameter uncertainty through the model and obtain prediction intervals, a Monte Carlo sampling approach is employed. Parameter uncertainty is characterized by assuming that the log-transformed parameters follow a multivariate normal distribution, with means given by the global point estimates and a covariance matrix estimated in log-space. Whenever possible, we approximate this covariance by the inverse of the observed Hessian of the negative log-likelihood at the optimum:

\begin{equation}
H(\hat{\theta}) = \left. \frac{\partial^2 \mathcal{L}(\theta)}{\partial \log_{10}(\theta)\,\partial \log_{10}(\theta)^T} \right|_{\theta = \hat{\theta}},
\label{eq:hessian}
\end{equation}

and define a regularised covariance

\begin{equation}
\hat{\Sigma}_H = \left( H(\hat{\theta}) + \lambda I \right)^{-1},
\label{eq:hessian_cov}
\end{equation}

where $I$ is the identity matrix and $\lambda$ is a small ridge term added for numerical stability. This full covariance captures parameter correlations (e.g.\ trade-offs between absorption and elimination rates) implied by the curvature of the likelihood. In cases where the numerical Hessian is unreliable or fails (e.g.\ due to ill-conditioning), we fall back to a diagonal covariance whose entries are given by the jackknife variances in log-space,

\begin{equation}
\hat{\Sigma}_{\text{diag}} = \mathrm{diag}\left(\sigma_{\text{jack},1}^2, \ldots, \sigma_{\text{jack},p}^2\right),
\label{eq:covariance_diag}
\end{equation}

which provides a conservative, axis-aligned approximation that remains well-conditioned even when the number of fitted parameters approaches the number of animals.

Thus, for Monte Carlo sampling we assume
...
where $\hat{\Sigma}$ is either $\hat{\Sigma}_H$ (preferred) or $\hat{\Sigma}_{\text{diag}}$ (fallback), and $p$ is the number of fitted parameters. A total of $M = 10{,}000$ parameter sets are sampled from this multivariate normal distribution in log-space and subsequently transformed to linear space:
...
For each sampled parameter set $\theta^{(m)}$, the model is simulated for all animals in the dataset, generating time-series predictions for each compartment. The resulting ensemble of predictions is used to compute empirical uncertainty intervals.

For each compartment $i$ and time point $t$, the prediction median and 95\% uncertainty interval are computed as:

\begin{align}
\text{Median}_{i,t} &= \text{median}\left( \{y_{i,t}^{(m)} : m = 1, \ldots, M\} \right), \\
\text{UI}_{i,t}^{2.5\%} &= \text{percentile}\left( \{y_{i,t}^{(m)} : m = 1, \ldots, M\}, 2.5 \right), \\
\text{UI}_{i,t}^{97.5\%} &= \text{percentile}\left( \{y_{i,t}^{(m)} : m = 1, \ldots, M\}, 97.5 \right),
...
The Monte Carlo procedure also allows us to decompose uncertainty into parameter, inter-animal, and observational components. Let $C_{ij}(t)$ denote the concentration for animal $j$ in compartment $i$ at time $t$, and write our model schematically in log-space as
\[
\log C_{ij}(t) = \underbrace{\mu_{i,t}(\theta)}_{\text{population mean at } t}
\;+\;
\underbrace{\eta_{ij,t}}_{\text{inter-animal deviation at } t}
\;+\;
\underbrace{\xi_{ij,t}}_{\text{measurement noise at } t}.
\]
Using the law of total variance, the variance of $\log C_{ij}(t)$ can be decomposed as
\[
\operatorname{Var}(\log C_{ij}(t))
=
\underbrace{\operatorname{Var}_\theta\!\big[\mu_{i,t}(\theta)\big]}_{\text{parameter (epistemic) uncertainty}}
+
\underbrace{\mathbb{E}_\theta\!\big[\operatorname{Var}(\eta_{ij,t}\mid\theta)\big]}_{\text{biological (inter-animal) variability}}
+
\underbrace{\mathbb{E}_{\theta,\eta}\!\big[\operatorname{Var}(\xi_{ij,t}\mid\theta,\eta_{ij,t})\big]}_{\text{measurement variability}}.
\]

We approximate these three components using Monte Carlo samples from the estimated distribution of $\theta$, together with simulated random effects and observation noise. This yields three nested 95\% intervals at each time point $t$:

\begin{itemize}
    \item \emph{Parameter-only interval:}
    For each Monte Carlo sample of $\theta$, we compute the population (animal-averaged) prediction $\mu_{i,t}(\theta)$. The 2.5th and 97.5th percentiles of these trajectories across all samples define an interval reflecting uncertainty in the population mean trajectory due solely to parameter uncertainty (epistemic uncertainty).

    \item \emph{Parameter plus inter-animal variation interval:}
    For each Monte Carlo sample, we additionally draw individual-specific random effects $\eta_{ij}$ and compute animal-specific predictions $\mu_{i,t}(\theta) + \eta_{ij}$. The 2.5th and 97.5th percentiles over all animals and Monte Carlo samples define an interval for the distribution of true (noise-free) concentrations for a randomly chosen animal, capturing biological (aleatoric) variability in addition to parameter uncertainty.

    \item \emph{Observation-level prediction interval:}
    Finally, log-normal observational noise,
    $\xi_{ij,t} \sim \mathcal{N}(0,\sigma_i^2)$ (with $\sigma_i^2$ given by the compartment-specific log-error variance estimated in the Tobit framework, Eq.~\ref{eq:sigma}),
    is added in log-space to obtain pseudo-observations. The 2.5th and 97.5th percentiles of these pseudo-observations define an interval for the concentration expected from a single measurement on a random animal, combining parameter, biological, and measurement uncertainty.
\end{itemize}

Together, these intervals provide a hierarchical decomposition of predictive uncertainty into epistemic (parameter) and aleatoric (biological and measurement) components, mirroring the variance decomposition above.
...
The Fisher Information Matrix is computed from the sensitivity of model predictions to parameter perturbations. For a parameter vector $\theta$ estimated in $\log_{10}$ space, we sum contributions over compartments $c$ and observation time points $j$:

\begin{equation}
\mathcal{I}(\theta) = \sum_{c \in \mathcal{M}} \sum_{j=1}^{N_c} \frac{1}{\sigma_c^2} \left( \frac{\partial \log y_{c,j}}{\partial \log_{10} \theta} \right)^T \left( \frac{\partial \log y_{c,j}}{\partial \log_{10} \theta} \right)
\label{eq:fim}
\end{equation}

where $\mathcal{M}$ is the set of modeled compartments, $N_c$ is the number of observations in compartment $c$, $\sigma_c$ is the effective log-error estimate for compartment $c$ (Section~\ref{subsubsec:loss}), $y_{c,j}$ is the predicted concentration in compartment $c$ at time point $j$, and the partial derivatives are computed with respect to parameters in $\log_{10}$ space.
...
\end{align}
which capture the characteristic lactation pattern with an initial peak followed by exponential decline. For goats, $\text{potLac}$ is chosen such that peak yields are in the range of 3--4 kg/day.
...
Physiological parameters differ by breed, species and parity, reflecting differences in body size, milk production potential, and metabolic characteristics. Table~\ref{tab:physiology_params} provides the parameter values used for each breed-parity combination in goats.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l l c c c c c c}
        ...
        \bottomrule
    \end{tabular}
    \caption{Parameters used to solve the equations in the physiological submodel for goats (body weight and potential lactation).}
