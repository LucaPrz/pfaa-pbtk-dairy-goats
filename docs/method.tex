...
To propagate parameter uncertainty through the model and obtain prediction intervals, a Monte Carlo sampling approach is employed. Parameter uncertainty is characterized by assuming that the log-transformed parameters follow a multivariate normal distribution, with means given by the global point estimates and a diagonal covariance matrix whose entries are given by the jackknife variances in log-space:

\begin{equation}
\hat{\Sigma}_{\text{diag}} = \mathrm{diag}\left(\sigma_{\text{jack},1}^2, \ldots, \sigma_{\text{jack},p}^2\right),
\end{equation}
Only the diagonal is used for Monte Carlo sampling to avoid unstable draws from a poorly conditioned full covariance when $n$ is small.

so that

\begin{equation}
\log_{10}(\theta) \sim \mathcal{N}\left( \log_{10}(\hat{\theta}), \hat{\Sigma}_{\text{diag}} \right)
\label{eq:mc_dist}
\end{equation}

where $p$ is the number of fitted parameters. For diagnostic purposes, the full jackknife covariance matrix can also be computed as
...
where $\mathbf{z}_{(-i)} = \log_{10}(\hat{\theta}_{(-i)})$ is the log-transformed parameter vector from jackknife iteration $i$, and $\bar{\mathbf{z}} = \frac{1}{n} \sum_{i=1}^{n} \mathbf{z}_{(-i)}$ is the mean log-transformed parameter vector. This full covariance matrix can reveal parameter correlations (e.g., trade-offs between absorption and elimination rates) but is not used directly for Monte Carlo sampling when the number of animals is small and the covariance is poorly conditioned.
...
The Monte Carlo procedure also enables separation of parameter uncertainty from inter-animal variability and observational noise. For each parameter set, predictions are generated across all animals, allowing computation of animal-specific means and standard deviations. Three nested 95\% intervals are defined:

\begin{itemize}
    \item A \emph{parameter-only} interval, obtained as the 2.5th and 97.5th percentiles of the animal-averaged predictions across all Monte Carlo samples. This interval reflects uncertainty in the population mean trajectory due to parameter uncertainty.
    \item A \emph{parameter plus inter-animal variation} interval, obtained as the 2.5th and 97.5th percentiles of all animal-specific predictions across all Monte Carlo samples. This interval represents the distribution of true (noise-free) concentrations for a randomly chosen animal.
    \item An \emph{observation-level} prediction interval, obtained by adding log-normal observational noise with variance $\Sigma_i^2$ (estimated from the Tobit fit) to the animal-specific predictions in log-space and computing the 2.5th and 97.5th percentiles of the resulting pseudo-observations. This interval quantifies the expected range of single measured concentrations for a random animal.
\end{itemize}

Together, these three intervals provide a stacked decomposition of uncertainty into contributions from parameter uncertainty, inter-animal variability, and observational error.
